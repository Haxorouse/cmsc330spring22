...
let rec regexp_to_nfa (regex: regexp_t) : (int, char) nfa_t =
  ...
  |Union (x, y) -> 
    ...
    (match nfaX.fs with xf::_ ->
    (match nfaY.fs with yf::_ ->
    let newF = fresh() in
    let newS = fresh() in
    {sigma = insert_all nfaX.sigma nfaY.sigma; q0 = newS; fs = [newF]; qs = insert_all [newS;newF] (insert_all nfaX.qs nfaY.qs); delta = insert_all [(newS, None, nfaX.q0); (newS, None, nfaY.q0); (xf, None, newF); (yf, None, newF)] (insert_all nfaX.delta nfaY.delta)}
  ...
  |Star reg -> 
    ...
    (match n.fs with | oldF::_ -> 
    let newF = fresh() in
    let newS = fresh() in
    {sigma = n.sigma; q0 = newS; fs = [newF]; qs = insert_all n.qs [newS;newF]; delta = insert_all [(newS, None, n.q0); (newS, None, newF); (oldF, None, newF); (oldF, None, n.q0)] n.delta}
    | [] -> failwith "unreachable");;
