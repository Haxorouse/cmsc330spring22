...
(*process list of transitions recursively to build list of outputs*)
  let rec add (edges: ('q, 's) transition list) (qs: 'q list) (s: 's option) : 'q list =
    ...
    ending::add xt qs s else
    add xt qs s
  in
  match s with
  | Some c ->
    if elem c nfa.sigma then
      add nfa.delta qs s
    else []
  | None -> add nfa.delta qs s;;
...
let e_closure (nfa: ('q,'s) nfa_t) (qs: 'q list) : 'q list =
  ...
  in
   process (insert_all(move nfa qs None) qs);;
...
let new_states (nfa: ('q,'s) nfa_t) (qs: 'q list) : 'q list list =
  ...
  in*)
  (*let sigMoves = List.fold_left (fun out alpha -> (move nfa qs (Some alpha))::out) [] nfa.sigma in
  let epsMoves = List.fold_left (fun out state -> (e_closure nfa state)::out) [] sigMoves in
  insert_all epsMoves sigMoves;;*)
